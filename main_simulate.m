...
umax = 3*plant.m*plant.g*2;           % control limit
...
elseif sim_config.include_obs == 1
    if use_distModel_in_planning_control == 1
        file_traj = 'nomTraj_w_obs_w_learning.mat';  
    else
        file_traj = 'nomTraj_w_obs_no_learning.mat';  
    end
end
...
Klp = [500*ones(5,1);200;200]; % 200 rad/s is for filtering estimated uncertainties. 
% tic;

% ---------------- ode23 is fastest, followed by ode45 ------------------
% [times,xuTraj] = ode23(@(t,xu) pvtol_dyn(t,xu,Klp,plant,controller,sim_config,dist_config,distEst_config),[0 duration],x_xhat_u_d_0); %,ode_opts)
% -----------------------------------------------------------------------
...
scatter(xnomTraj(1,end),xnomTraj(1,end),'k*')
...
if sim_config.save_sim_rst == 1
    ...
    if use_distModel_in_planning_control == 1
        file_name = [file_name '_with_true_nn_'];
    end
...
%% get dist data
% dist = zeros(2,simuLen);
% for i = 1:simuLen
%     xx = xTraj(:,i);
%     dist(:,i) = actual_dist_fcn(xx,dist_config.center,dist_config.radius);
% end
% 
...
% xTraj_trsp = xTraj';
% dist_trsp = dist';
% save('states_60_7.510.mat','xTraj_trsp');
% save('disturbance_60_7.510.mat','dist_trsp');
% disp('finish')
...
% %             d_force_dx(:,:,i) = zeros(2,n);
% %         elseif distance_to_center(i)<=radius-1
% %             d_force_dx(:,4:5,i) = [-1;-1]*dist_intensity(i)*2*max_damping*x(4:5,i)'; % note that dist_intensity(i) equal to 1 under this scenario
% %         else
% %             d_force_dx(1,1:2,i) = 0.5*sin(pi*(radius-distance_to_center(i)-1))*(-pi)/distance_to_center(i)*dist_force_max(i)*(x(1:2,i)-center)';
% %             d_force_dx(1,4:5,i) = -dist_intensity(i)*2*max_damping*x(4:5,1)';
% %             d_force_dx(2,:,i) = d_force_dx(1,:,i);
% %         end
%         
%         % ---------------- using a ReLU function --------------------
% %         if distance_to_center(i)>radius
% %             d_force_dx(:,:,i) = zeros(2,n);
% %         else
% %             d_force_dx(1,1:2,i) = 1/radius./distance_to_center(i)*dist_force_max(i)*(x(1:2,i)-center)';
% %             d_force_dx(1,4:5,i) = -dist_intensity(i)*2*max_damping*x(4:5,1);
% %             d_force_dx(2,:,i) = d_force_dx(1,:,i);
% %         end
%        
%         % ---------------- using a linear function --------------------
% %         d_force_dx(:,1:2,i) = [-1;-1]*(1/radius/distance_to_center(i)*dist_force_max(i)*x(1:2,i)');
...
if nargout>1
    ...
    for i = 1:N        
        % ---------------- using an inverse function -----------------
%         xx = x(:,i);
        xx_reduced = x([1 2 4 5],i)';

%         tic;
        jac = jacobian(prd_dist,xx_reduced);
%         toc;
        d_force_dx(:,1:2,i) = jac(:,1:2)';
        d_force_dx(:,4:5,i) = jac(:,3:4)';
    end
end
% toc;
